...
real,allocatable,dimension(:,:)     ::  V_lr 
integer                             ::  i_x,i_s,i2,i3,ip,is,ind,iv,ns 
...
double precision        :: rcell_sub_charge(3,3),rcell_sub_neutral(3,3),r_0(3)
...
!integer               ::      nsm
!-----------------------------------------------------------------------
real(kind=8) ::q_model,ev_to_k,q_model_r
!-----------------------------------------------------------------------
...
!Calculating V^{lr}
!-----------------------------------------------------------------------
!allocate(V_lr(maxp_sub_charge,nspin_sub_charge))
!do is = 1,ns
!	ind = 0
!	do i3 = 1,mesh_sub_charge(3)
!		do i2 = 1,mesh(2)
!			do ip,mesh_sub_charge(1)
!				V_lr(ind+ip,is)=q_model_r(x,gamma2,beta2,g2,r_0)
...
!-----------------------------------------------------------------------
real(kind=8) Function q_model_r(x,gamma_d,beta_d,r_0,r)
     use units,   only : pi
!    "Gaussian Model in PyCDT"
!    "q(r)=q[x exp(-r/gamma) +(1-x) exp(-r^2/beta^2)]"
!    "q(g2)=x/sqrt(1+gamma^2*g2)+*(1-x)*exp^(-beta2*g2) "
...
!    OUTPUT    q_model: Charge density at real magnitude 
     implicit none
     real(kind=8)          :: gamma_d,beta_d,x,N_gamma,N_beta
     real(kind=8)          :: r,r_0 
     N_gamma=8*pi*gamma_d**3
     N_beta=pi**(3.0/2.0)*beta_d**3
     q_model_r=x *N_gamma**(-1)*exp(abs(r-r_0)/beta_d)+(1-x)*N_beta**(-1)*exp(abs(r-r_0)**2/beta_d**2)
